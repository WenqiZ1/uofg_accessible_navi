...
document.getElementById('styleSwitcher').addEventListener('change', function () {
  ...
  map.once('styledata', async () => {
    ...
    ['all','step'].forEach(kind => {
      ...
      map.addLayer({
        id: cfg.layerId, type:'line', source: cfg.sourceId,
...
/* ===================== Landmarks (bilingual descriptions) ===================== */
const landmarks = [
  ...
  { name: "Bower Building", coord: [-4.29167, 55.87245],
    description:{en:"Teaching and research facility for the School of Life Sciences.", zh:"生命科学学院的教学与科研楼。"},
    elevator:"Yes", ramp:"Yes", accessible_toilet:"Yes", type:"campus" },
  ...
  { name: "Kelvin Building", coord: [-4.29130, 55.87175],
    description:{en:"Home to Physics & Astronomy; north side of University Avenue.", zh:"物理与天文学所在地；位于大学大道北侧。"},
    elevator:"Yes", ramp:"Yes", accessible_toilet:"No", type:"campus" },
  ...
  { name: "Advanced Research Building", coord: [-4.29580, 55.87145],
    description:{en:"Modern facility dedicated to interdisciplinary research and innovation.", zh:"致力于跨学科研究与创新的现代化设施。"},
    elevator:"Yes", ramp:"Yes", accessible_toilet:"Yes", type:"campus" },
  ...
  { name: "McMillan Reading Room", coord: [-4.28801, 55.87271],
    description:{en:"Quiet study space between the Main Building and the Library.", zh:"主楼与图书馆之间的安静自习空间。"},
    elevator:"Yes", ramp:"Yes", accessible_toilet:"Yes", type:"campus" },

  { name: "Wolfson Medical School Building", coord: [-4.29310, 55.87302],
    description:{en:"Dedicated to medical education with advanced simulation facilities.", zh:"拥有先进模拟设施的医学教育大楼。"},
    elevator:"Yes", ramp:"Yes", accessible_toilet:"Yes", type:"campus" },

  /* Museums */
  { name: "Hunterian Art Museum", coord: [-4.28835, 55.87180],
    description:{en:"Scotland's oldest public museum, within the Gilbert Scott Building.", zh:"苏格兰最古老的公立博物馆，位于 Gilbert Scott Building 内。"},
    elevator:"Yes", ramp:"Yes", accessible_toilet:"No", type:"museum" },

  { name: "Kelvingrove Art Gallery and Museum", coord: [-4.29110, 55.86850],
    description:{en:"Major civic museum beside Kelvingrove Park with art and natural history collections.", zh:"位于凯尔文格罗夫公园旁的大型市民博物馆，收藏艺术与自然历史。"},
    elevator:"Yes", ramp:"Yes", accessible_toilet:"Yes", type:"museum" },

  /* Transport */
  { name: "Kelvinhall Station", coord: [-4.29969, 55.87102],
    description:{en:"Glasgow Subway station serving the West End.", zh:"服务于西区的格拉斯哥地铁站。"},
    elevator:"No", ramp:"No", accessible_toilet:"No", type:"transport" },

  { name: "Hillhead Subway Station", coord: [-4.29329, 55.87519],
    description:{en:"Circle line station on Byres Road; step-free from street to concourse.", zh:"拜尔斯路环线车站；从街道到大厅无台阶。"},
    elevator:"Partial/Assisted", ramp:"Yes", accessible_toilet:"No", type:"transport" },

  { name: "Partick Station", coord: [-4.30939, 55.87],
    description:{en:"Major hub connecting ScotRail, Subway and buses.", zh:"连接苏格兰铁路、地铁与公交的交通枢纽。"},
...
function addLegend() {
  ...
  legend.style.cssText = 'position:absolute;bottom:16px;left:16px;background:#fff;padding:8px 10px;border-radius:8px;font:12px/1.4 system-ui, sans-serif;box-shadow:0 1px 4px rgba(0,0,0,.15)';
  legend.innerHTML = `
...
function setLang(lang) {
  ...
  if (subEl)   subEl.textContent   = U.subtitle;
  ...
  const sI = document.getElementById('start'); if (sI) sI.placeholder = U.startPH;
  const eI = document.getElementById('end');   if (eI) eI.placeholder = U.endPH;
  const btn= document.getElementById('routeBtn'); if (btn) btn.textContent = U.plan;

  const la = document.getElementById('legendAll');  if (la) la.textContent = U.legendAll;
  const ls = document.getElementById('legendStep'); if (ls) ls.textContent = U.legendStep;
